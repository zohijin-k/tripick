/**
 * 코스 신뢰도(Trust Score)를 100점 만점으로 계산합니다.
 *
 * 구성:
 *   완주율      35%
 *   리뷰 신뢰도 25%  (리뷰 수 15 + 평균 별점 10)
 *   수행자 수   20%  (log 스케일)
 *   GPS 검증    10%
 *   데이터 품질 10%
 *
 * @param {object} course
 * @param {Array}  reviews - getReviewsForCourse()로 가져온 리뷰 배열 (기본값 [])
 * @returns {{ score: number, items: Array }}
 */
export function calculateTrustScore(course, reviews = []) {
  const spots = course.spots ?? [];

  // 1. 완주율 (35점 만점)
  const completionVal = Math.round(((course.completionRate ?? 0) / 100) * 35);

  // 2. 리뷰 신뢰도 (25점 만점: 리뷰 수 15 + 별점 10)
  ...
  // 3. 수행자 수 (20점 만점, log 스케일 – 100명 기준 만점)
  ...
  const gpsVal = spots.length > 0 ? Math.round((gpsValid / spots.length) * 10) : 0;
